...
export async function getModule(
  ...
  return modules.find((m) => m.id === contentModuleId) || null
}
...
export async function getLessonsForModule(
  ...
  const contentModuleDir = path.join(contentDir, trackId, moduleId)
  
  if (!fs.existsSync(contentModuleDir)) {
    return []
  }
  
  const files = fs.readdirSync(contentModuleDir).filter((f: string) => f.endsWith('.json'))
  const lessons: Lesson[] = []
  
  for (const file of files) {
    const lessonFile = path.join(contentModuleDir, file)
...
> {
  ...
  for (const track of tracks) {
    for (const contentModule of track.modules) {
...
export async function getNextLesson(
  ...
  for (const contentModule of track.modules) {
    for (const lesson of contentModule.lessons) {
      if (foundCurrent) {
        return { lesson, moduleId: contentModule.id }
      }
...
export async function getPreviousLesson(
  ...
  for (const contentModule of track.modules) {
    for (const lesson of contentModule.lessons) {
      ...
      previous = { lesson, moduleId: contentModule.id }
    }
  }
